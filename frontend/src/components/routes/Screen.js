...
class Screen extends Component {
    ...
    handleTime() {
        // Timer paused
        if (!this.state.playing || this.state.display_step > 0) return;

        // Update timer
        let time = this.state.time;
        if (
            this.state.playing &&
            this.state.quizz.questions[this.state.room.step].time >
                this.state.time
        )
            time += 1;
        ...
        const limit =
            ...
            this.state.quizz.questions[this.state.room.step].time;
        let color = '';
        if (limit < 50) color = 'info';
        else if (limit < 75) color = 'warning';
        else color = 'danger';
            ...
            .then(players => {
                players.sort(function(a, b) {
                    if (parseInt(a.score, 10) > parseInt(b.score, 10))
                        return -1;
                    if (parseInt(a.score, 10) < parseInt(b.score, 10)) return 1;
                    return 0;
                });
